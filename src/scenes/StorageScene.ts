/**
 * StorageScene — personal chest storage UI.
 *
 * Launched when the player presses E facing a chest tile whose metadata has
 * `storage: true` (i.e. the personal house chest).  Unlike loot chests, a
 * storage chest is never emptied — items persist in the tile's metadata in
 * Firebase and can be freely deposited/withdrawn between sessions.
 *
 * Layout:
 *   Left panel  — chest contents; click to withdraw to inventory
 *   Right panel — player inventory;  click to deposit into chest
 *
 * All changes are immediately written to Firebase (tile + player inventory).
 */
...
export class StorageScene extends Phaser.Scene {
  ...
  init(data: StorageSceneData): void {
    this._data  = data
    const tile  = getTile(data.tileX, data.tileY)
  ...
  private _buildOverlay(): void {
    ...
    this._overlay.innerHTML = `
      <div id="st-box">
        <div id="st-header">
          <span id="st-title">Personal Storage</span>
  ...
  private _renderChest(): void {
    ...
    if (!this._chest.length) {
      grid.innerHTML = '<div class="st-empty">Empty</div>'
      return
    }
    ...
    this._chest.forEach((s, i) => {
      const canvas = grid.querySelector<HTMLCanvasElement>(`.st-cell[data-i="${i}"] .st-icon`)
  ...
  private _renderBag(): void {
    ...
    grid.querySelectorAll<HTMLElement>('.st-cell').forEach(cell => {
      cell.addEventListener('click', () => {
        // find the real inventory index (inv is filtered — re-look up by position)
  ...
  private _withdraw(chestIdx: number): void {
    const item = this._chest[chestIdx]
    if (!item) return
  ...
  private _persist(): void {
    const player   = getLocalPlayer()
    const existing = getTile(this._data.tileX, this._data.tileY)
    if (!existing) return
